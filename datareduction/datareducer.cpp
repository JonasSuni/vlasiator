...
void initializeDataReducers(DataReducer * outputReducer, DataReducer * diagnosticReducer)
{
   ...
   for (it = P::outputVariableList.begin();
      ...
      */
      ...
      if(P::systemWriteAllDROs || lowercase == "fg_e_djdt") {
         for(int index=0; index<fsgrids::N_EDJDT; index++) {
            std::string reducer_name = "fg_e_djdt_" + std::to_string(index);
            outputReducer->addOperator(new DRO::DataReductionOperatorFsGrid(reducer_name,[index](
                         FsGrid< std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH> & perBGrid,
                         FsGrid< std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH> & EGrid,
                         FsGrid< std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH> & EHallGrid,
                  ...
                  // Iterate through fsgrid cells and extract EdJdt
                  for(int z=0; z<gridSize[2]; z++) {
                     for(int y=0; y<gridSize[1]; y++) {
                        for(int x=0; x<gridSize[0]; x++) {
                           retval[gridSize[1]*gridSize[0]*z + gridSize[0]*y + x] = (*EDJdtGrid.get(x,y,z))[index];
                        }
                     }
                  }
                  return retval;
            ...
            outputReducer->addMetadata(outputReducer->size()-1,"V/m","$\\mathrm{V}\\,\\mathrm{m}^{-1}$","$E_\\mathrm{dJ/dt,"+std::to_string(index)+"}$","1.0");
         }
         if(!P::systemWriteAllDROs) {
            continue;
         }
...
bool DataReducer::writeFsGridData(
   ...
   if(!DROf) {
      ...
      return DROf->writeFsGridData(perBGrid, EGrid, EHallGrid, EGradPeGrid, EDJdtGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, volGrid, technicalGrid, meshName, vlsvWriter, writeAsFloat);
   }
}
