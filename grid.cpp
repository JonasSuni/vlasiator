   ...
   if (P::isRestart) {
      ...
      if (P::forceRefinement) {
         ...
         for (int i = 0; i < P::amrMaxSpatialRefLevel; ++i) {
            ...
            if (!adaptRefinement(mpiGrid, technicalGrid, sysBoundaries, project, false, i)) {
         ...
         balanceLoad(mpiGrid, sysBoundaries, technicalGrid);
         adaptRefinement(mpiGrid, technicalGrid, sysBoundaries, project, false);
         balanceLoad(mpiGrid, sysBoundaries, technicalGrid);
      }
   }
...
bool adaptRefinement(dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH> & technicalGrid, SysBoundary& sysBoundaries, Project& project, bool firstPass, int useStatic) {
   ...
   // Added a bit of magic (empirical) safety overhead
   for (auto id : mpiGrid.get_local_cells_to_unrefine()) {
      newBytes += 1.47 * mpiGrid[id]->get_cell_memory_capacity();
   }
