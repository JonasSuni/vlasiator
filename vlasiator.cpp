 ...
 */
// Optimize cell counts reporting with pre-allocated vectors
void report_cell_and_block_counts(dccrg::Dccrg<spatial_cell::SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid) {
    static vector<int64_t> localCounts, globalCounts;
    ...
    if (localCounts.size() == 0) {
        localCounts.resize((popCount+1)*(maxRefLevel+1), 0);
        globalCounts.resize((popCount+1)*(maxRefLevel+1), 0);
    }
...
// Check whether diagnostic output has to be produced
        ...
        #pragma omp parallel
        {
            threadLocalCells = getLocalCells();
            // Use thread local copy in calculations
        }
