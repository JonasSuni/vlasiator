...
void calculateEdgeDJdtTermXComponents(
   ...
   if (Parameters::dJdt_coeff == 0) {
      cerr << __FILE__ << __LINE__ << "You shouldn't be in a time derivative of current term function if Parameters::ohmGradPeTerm == 0." << endl;
   } else {
      ...
      EDJdtGrid.get(i,j,k)->at(fsgrids::edjdt::EXDJDTE) = Parameters::dJdt_coeff /
      limitedRhoq /
...
void calculateEdgeDJdtTermYComponents(
   FsGrid< std::array<Real, fsgrids::edjdt::N_EDJDT>, FS_STENCIL_WIDTH> & EDJdtGrid,
   FsGrid< std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH> & momentsGrid,
   FsGrid< std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH> & dPerBGrid,
   FsGrid< std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH> & dPerBOldGrid,
   ...
   if (Parameters::dJdt_coeff == 0) {
      ...
      rhoq = momentsGrid.get(i,j,k)->at(fsgrids::moments::RHOQ);
      limitedRhoq = (rhoq <= Parameters::hallMinimumRhoq ) ? Parameters::hallMinimumRhoq : rhoq ;
      EDJdtGrid.get(i,j,k)->at(fsgrids::edjdt::EYDJDTE) = Parameters::dJdt_coeff /
      limitedRhoq /
      physicalconstants::MU_0 *
      physicalconstants::MASS_ELECTRON /
      physicalconstants::CHARGE *
...
void calculateEdgeDJdtTermZComponents(
   FsGrid< std::array<Real, fsgrids::edjdt::N_EDJDT>, FS_STENCIL_WIDTH> & EDJdtGrid,
   FsGrid< std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH> & momentsGrid,
   FsGrid< std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH> & dPerBGrid,
   FsGrid< std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH> & dPerBOldGrid,
   ...
   if (Parameters::dJdt_coeff == 0) {
      ...
      rhoq = momentsGrid.get(i,j,k)->at(fsgrids::moments::RHOQ);
      limitedRhoq = (rhoq <= Parameters::hallMinimumRhoq ) ? Parameters::hallMinimumRhoq : rhoq ;
      EDJdtGrid.get(i,j,k)->at(fsgrids::edjdt::EZDJDTE) = Parameters::dJdt_coeff /
      limitedRhoq /
      physicalconstants::MU_0 *
      physicalconstants::MASS_ELECTRON /
      physicalconstants::CHARGE *
...
void calculateDJdtTermSimple(
   ...
   phiprof::Timer DJdtTimer {"Calculate DJdt term"};
   int computeTimerId {phiprof::initializeTimer("EDJdt compute cells")};
